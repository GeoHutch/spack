...
.. code-block:: python
   ...
   if 'avx512' in spec.architecture.target:
...
.. code-block:: python
   ...
   if spec.architecture.target.family == 'ppc64le':
       args.append('--enable-power')

Possible values for the ``family`` attribute are displayed by ``spack arch`` under the
"Generic architectures" header.
...
.. code-block:: python
   ...
   if target.microarchitecture > llnl.util.cpu.targets['haswell']:
...
.. admonition:: Using Spack on unknown microarchitectures
   ...
   for their work. The software built won't be probably as optimized as it could, but it's
   very likely that information on this unknown architecture will be added to the next
   version of Spack.
